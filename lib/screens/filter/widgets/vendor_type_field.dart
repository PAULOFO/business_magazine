      ...
      builder: (state) {
        return Row(
          children: <Widget>[
            GestureDetector(
              onTap: () {
               if(state.value & VENDOR_TYPE_PARTICULAR != 0){//Para verificar
                if(state.value & VENDOR_TYPE_PROFESSIONAL != 0)
                  state.didChange(state.value & ~VENDOR_TYPE_PARTICULAR);//P desabilitar uma Bit Flag em uma váriavel
                else
                  state.didChange(VENDOR_TYPE_PROFESSIONAL);
                } else {
                  state.didChange((state.value | VENDOR_TYPE_PARTICULAR));//Para habilitar
                }//didChange é o equivalente ao setState
              },
              child: Container(
                height: 50,
                width: 120,
            ...
            GestureDetector(
              onTap: () {
                if(state.value & VENDOR_TYPE_PROFESSIONAL != 0){//Para verificar
                  if(state.value & VENDOR_TYPE_PARTICULAR != 0)
                    state.didChange(state.value & ~VENDOR_TYPE_PROFESSIONAL);//P desabilitar uma Bit Flag em uma váriavel
                  else
                    state.didChange(VENDOR_TYPE_PARTICULAR);
                } else {
                  state.didChange((state.value | VENDOR_TYPE_PROFESSIONAL));//Para habilitar
                }//didChange é o equivalente ao setState
              },
              child: Container(
                height: 50,
